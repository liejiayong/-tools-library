...
export default class Orient {
  ...
  constructor(config) {
    ...
    window.addEventListener('resize', () => {
      ...
      this.$parent.style.cssText = Orient._getParentStyle(this.modeStat, Orient.DEFAULTS.modal.zIndex)
    })
    this.modeStat = Orient._detectOrient()
    // console.log(this.$parent)
    this.$parent.style.cssText = Orient._getParentStyle(this.modeStat, Orient.DEFAULTS.modal.zIndex)
    Orient._createTipsStyle(Orient.DEFAULTS.modal.zIndex)
    Orient._createTipsDOM({ ...Orient.DEFAULTS.modal, mode: this.modeStat, iconURL: this.icon.current })
  }
  /**
   * 设置icon
   * @param {String} param0 
