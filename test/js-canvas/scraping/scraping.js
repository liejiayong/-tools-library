...
class Scraping {
	...
	setBG(url = '') {
		this._loadImg(url).then(({ width, height, image }) => {
			...
			console.log(cw, ch, imgW, imgH)
			this.ctx.drawImage(image, left, top, imgW, imgH)
		})
	}
	/**
	 * 设置奖品
	...
	_createCanvas() {
		...
		if (width && height) {
			...
			cw = _getBoundingClientRectInfo(this.el).width
			ch = _getBoundingClientRectInfo(this.el).height
		}
