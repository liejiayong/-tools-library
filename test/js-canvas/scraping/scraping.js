 ...
 * canvas 刮刮卡
 */
function _getBoundingClientRectInfo(element) {
	let rect = null
...
class Scraping {
	...
	setBG(url = '') {
		this._loadImg(url).then(({ width, height, image }) => {
			...
			if (this.config.scale) {
				const scale = 1 / this.config.pixelRatio,
					sw = width * scale,
					sh = height * scale
			...
			console.log(cw, ch, width, height, left, top)
	...
	setAward() {
		// 奖品背景
		this.setBG(this.config.awardImg)

		// 图层背景
		if (this.config.coverImg) {
			this.setBG(this.config.coverImg)
		} else {
			// 如果没设置图片涂层，则使用纯色涂层
	...
	_createCanvas() {
		...
		const b = _getBoundingClientRectInfo(this.el)
		...
		if (getStyle(canvas, 'width') || getStyle(canvas, 'height')) canvas.style.cssText = 'width:initial;height:initial'
	...
	_eventMove(e) {
		...
		if (this.isDown) {
			...
			console.log(x,y, pixelRatio, radius)
			this.ctx.beginPath()
