...
Component({
  ...
  properties: {
    ...
    isliked: {
      ...
      observer(status, oldVal, changedPath) {
        this.setData({ status }, () => {
          // console.log('tab-like status', status)
          this.initImageUrl()
          this.initStyle()
          this.initTextCls()
        })
  ...
  methods: {
    handletab({ currentTarget: { dataset: { postid }}}) {
      const { count } = this.data

      this.data.status = !this.data.status

      this.data.status ? this.setData({ countStatus: +this.data.countStatus + 1 }) : this.setData({ countStatus: +this.data.countStatus - 1 })
      this.initImageUrl()
      this.initStyle()
      this.initTextCls()
