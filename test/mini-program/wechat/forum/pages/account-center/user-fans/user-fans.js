...
const { TYPE_CONFIG_FOCUS_BY_SUBSCRIBE } = require('../../../config/account-center-config')
...
Page({
  ...
  getComment(page = this.data.page) {
    ...
    getMyFocus({
      page,
      type: TYPE_CONFIG_FOCUS_BY_SUBSCRIBE,
      ...
      const fansList = this.data.fansList.concat(data)
      this.setData({ fansList, isPullUp: false, isPullDown: false, isUnConnect: false })
      this.data.page++
    }).catch(() => {
      this.data.isLoading = false
