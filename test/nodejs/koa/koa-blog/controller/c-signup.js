...
exports.postSignup = async ctx => {
  ...
  await findDataCountByName(name).then(async result => {
    ...
    if (result[0].count >= 1) {
      ...
      if (upload) {
        await registerUser([name, md5(password), getName + '.png', moment().format('YYYY-MM-DD HH:mm:ss')]).then(res => {
