...
const cancelToken = {
  ...
  set(req) { // 设置记录
    ...
    req.cancelToken = new axios.CancelToken(cancel => {
      ...
      this.record.push({ u: req.url + '&' + req.method, cancel })
    })
  }
}
...
http.interceptors.request.use(
  config => {
    setBaseURL(config)
    //是否存在token
    var token = store.state.user.token || cookie.getItem(COOKIES_TYPE_TOKEN)
    //是否需要登陆权限
    var requiresAuth = cookie.getItem(COOKIES_TYPE_REQUIRE_AUTH)
    // 设置cancelToken
...
http.interceptors.response.use(
  response => {
    ...
    if (code === 50001 || code === 50002 || code === 50003 || code === 50004) {
      ...
      cookie.removeItem(COOKIES_TYPE_TOKEN)
      cookie.removeItem(COOKIES_TYPE_USERINFO)
      router.push({
        name: 'auth_index',
        query: {
